...
class PlayState extends MusicBeatState
{
	...
	//custom character damn This is hard
	public static var whitty:Character;
	public static var tricky:Character;
	
	public static var tabi:Boyfriend;
	public static var agoti:Boyfriend;
    public static var whittyfe:Boyfriend;
	//custom character
	var tabising:Bool = false;
	var agotising:Bool = false;
	var whittysing:Bool = false;
	var trickysing:Bool = false;
	var whittyfsing:Bool = false;
	public static var noagoti:Boyfriend;
	public static var notabi:Boyfriend;
	public static var nowhitty:Character;
	public static var notricky:Character;
	public static var nowhittyfe:Boyfriend;
	//custom Background
	var top:FlxSprite;
	var middle:FlxSprite;
	var bottom:FlxSprite;
	var whittybg:FlxSprite;
	var agotibg:FlxSprite;
	var tabibg:FlxSprite;
	var line:FlxSprite;
	var tiky1:FlxSprite;
    var fire:FlxSprite;
	...
	override public function create()
	{
			...
			case 'mashup':
				{
					...
					curStage = 'mashup';
					//tabi layer
					top = new FlxSprite(-600, -200).loadGraphic(Paths.image('bgmash1'));
					...
					//tabi layer
					middle = new FlxSprite(-600, -200).loadGraphic(Paths.image('bgmash'));
					middle.antialiasing = true;
					middle.scrollFactor.set(0.9, 0.9);
					middle.active = false;
					
					//agoti layer
					bottom = new FlxSprite(-600, -200).loadGraphic(Paths.image('bgmash2'));
					bottom.antialiasing = true;
					bottom.scrollFactor.set(0.9, 0.9);
					bottom.active = false;

					tabi = new Boyfriend(700, 200, 'tabi');
		            agoti = new Boyfriend(1400, 500, 'agoti');
		            whitty = new Character(-100, 200, 'whitty');
					tricky = new Character(-600, 200, 'exTricky');
                    whittyfe = new Boyfriend(300, 200, 'whitty');
					//no animation play
					notabi = new Boyfriend(700, 200, 'tabi');
		            noagoti = new Boyfriend(1400, 500, 'agoti');
		            nowhitty = new Character(-100, 200, 'whitty');
					nowhittyfe = new Boyfriend(300, 200, 'whitty');
					notricky = new Character(-600, 200, 'exTricky');
					//noanimaiton
					tabi.alpha = 0;
				    agoti.alpha = 0;
				    whitty.alpha = 0;
			        ...
			        nowhittyfe.alpha = 0;
					//FIRE
					fire = new FlxSprite(700, 200);
					fire.frames = Paths.getSparrowAtlas('FireStage');
					fire.animation.addByPrefix('flame', 'FireStage', 24, false);
					...
					//some effect
					vignette = new FlxSprite(-600, -200).loadGraphic(Paths.image('vignette'));
					vignette.setGraphicSize(Std.int(vignette.width * 4));
					vignette.alpha = 0.4;
					red = new FlxSprite(-400, -100).loadGraphic(Paths.image('red'));
					red.setGraphicSize(Std.int(red.width * 3));
					red.alpha = 0.4;
				
					//tiky stage damn it so big
                    tiky1 = new FlxSprite(-2000, -400).loadGraphic(Paths.image('tiky1'));
					line = new FlxSprite(-2000, -400).loadGraphic(Paths.image('tiky5'));
		...
		/*if (SONG.player1 == "tabi")
			{
				if(FlxG.save.data.distractions)
					{
					var trail = new FlxTrail(tabi, null, 50, 3, 0.7, 0.089);
					add(trail);
					}
			}*/
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'tabinote':
					babyArrow.frames = Paths.getSparrowAtlas('tabi/NOTE_assets');
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						switch (Math.abs(daNote.noteData))
						{
							case 2:
								if (whittysing)
								whitty.playAnim('singUP' + altAnim, true);	
								else if (trickysing)
							...
							case 3:
								if (whittysing)
								whitty.playAnim('singRIGHT' + altAnim, true);
								else if (trickysing)
							...
							case 1:
								if (whittysing)
								whitty.playAnim('singDOWN' + altAnim, true);	
								else if (trickysing)
							...
							case 0:
								if (whittysing)
								whitty.playAnim('singLEFT' + altAnim, true);
								else if (trickysing)
						...
						daNote.active = false;
						if (SONG.song.toLowerCase()=="genocider")
						{
							if (health>0)
							{
								health -= 0.0090;
	...
	*/
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
			{
				...
				if (!note.wasGoodHit)
				{
					...
					switch (note.noteData)
					{
						case 2:
							if(tabising)
								tabi.playAnim('singUP', true);
							else if(agotising)
								agoti.playAnim('singUP', true);
							else if(whittyfsing)
								whittyfe.playAnim('singUP', true);
						    else
						    	boyfriend.playAnim('singUP', true);
						case 3:
							if(tabising)
								tabi.playAnim('singRIGHT', true);
							else if(agotising)
								agoti.playAnim('singRIGHT', true);
							else if(whittyfsing)
								whittyfe.playAnim('singRIGHT', true);
						    else
								boyfriend.playAnim('singRIGHT', true);
						case 1:
							if(tabising)
								tabi.playAnim('singDOWN', true);
							else if(agotising)
								agoti.playAnim('singDOWN', true);
							else if(whittyfsing)
								whittyfe.playAnim('singDOWN', true);
						    else
								boyfriend.playAnim('singDOWN', true);
						case 0:
							if(tabising)
								tabi.playAnim('singLEFT', true);
							else if(agotising)
								agoti.playAnim('singLEFT', true);
							else if(whittyfsing)
								whittyfe.playAnim('singLEFT', true);
						    else
	...
	override function stepHit()
	{
		...
		#end
		if (curStage == 'mashup' && curStep != stepOfLast)
		{
			switch(curStep)
		{
          case 127:
